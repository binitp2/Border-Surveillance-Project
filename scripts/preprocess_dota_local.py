...
Usage:
    cd "/home/binitpatel/Workspace/Border Surveillance Project"
...
BASE_DIR = Path("/home/binitpatel/Workspace/Border Surveillance Project")
...
DOTA_TRAIN_LBL_FALLBACKS = [
    ...
    DOTA_ROOT / "train" / "labelTxt-v1.0" /
    "Train_Task2_gt" / "trainset_reclabelTxt",
]
...
def process_dota_train() -> int:
    ...
    images = list(DOTA_TRAIN_IMG.glob("*.png")) + \
        list(DOTA_TRAIN_IMG.glob("*.jpg"))
...
def process_dota_val() -> int:
    ...
    images = list(DOTA_VAL_IMG.glob("*.png")) + \
        list(DOTA_VAL_IMG.glob("*.jpg"))
    print(f"   Found {len(images)} images")
    ...
    for split_name, files in splits.items():
        for img_path in tqdm(files, desc=f"   DOTA {split_name}", leave=False):
            ...
            if resize_and_save(img_path, dst_img):
                ...
                lbl_out = DATA_OUT / split_name / \
                    "labels" / (img_path.stem + ".txt")
                with open(lbl_out, "w") as f:
...
def print_summary():
    ...
    print()
    print('  cd "/home/binitpatel/Workspace/Border Surveillance Project"')
    print()
    print("  yolo train \\")
