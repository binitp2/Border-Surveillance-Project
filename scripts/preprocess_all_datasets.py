...
def process_xview() -> Stats:
    ...
    if not images_dir.exists():
        logger.warning(
            "xView images dir not found: %s  — skipping", images_dir)
        return stats
...
def verify_dataset() -> None:
    ...
    for split in ("train", "val", "test"):
        ...
        if orphan_imgs:
            logger.warning("%s: %d images without label",
                           split, len(orphan_imgs))
            issues += len(orphan_imgs)
        if orphan_lbls:
            logger.warning("%s: %d labels without image",
                           split, len(orphan_lbls))
    ...
    if issues == 0:
        ...
        logger.warning(
            "Verification found %d issue(s) — see log above", issues)
