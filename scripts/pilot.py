...
def stage3_anomaly(frame_results: list) -> list:
    ...
    try:
        ...
        for fr in frame_results:
            if baseline_count < MIN_SAMPLES:
                ...
                if ready and not fitted:
                    info(
                        f"Baseline collected ({MIN_SAMPLES} frames) — fitting...")
                    t0 = time.time()
                    detector.fit()
        ...
        if not fitted:
            warn(
                f"Only {baseline_count} frames — less than MIN_SAMPLES={MIN_SAMPLES}")
        ...
        criticals = sum(
            1 for r in anomaly_results if r.alert_level == "critical")
