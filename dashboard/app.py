...
Author: Border Surveillance AI Team (Binit Patel)
Date:   April 2026
"""
...
def _send_alert_email(alert: dict, to_email: str) -> tuple[bool, str]:
    ...
    reason = reasons[0] if isinstance(
        reasons, list) and reasons else str(reasons)
...
def chart_priority_donut(df: pd.DataFrame):
    ...
    fig.update_layout(
        **_base_layout(height=240, margin=dict(l=5, r=5, t=10, b=5)))
    return fig
...
def render_sidebar(df: pd.DataFrame, session: dict):
    with st.sidebar:
        ...
        <div style="font-size:0.72rem; color:{TEXT_DIM}; line-height:2">
            <div style="color:{TEXT_PRIMARY};font-weight:600;font-size:0.82rem;font-family:'Rajdhani',sans-serif;">Binit Patel</div>
            <div style="color:{CYAN};font-size:0.68rem;letter-spacing:1px;margin-bottom:4px">Azure Specialist · AI Engineer</div>
            <div>🏛 Government Engineering College, Bharuch</div>
            <div>🎓 B.E. Computer Engineering — Class of 2026</div>
            <div>📋 Enroll: 230143107014</div>
            <div>🏆 MS Elevate Internship 2026</div>
            <div style="margin-top:6px">
                <a href="https://in.linkedin.com/in/dte-gecbh-com-binitpatel" target="_blank"
                   style="color:{CYAN};text-decoration:none">🔗 LinkedIn</a>
                &nbsp;·&nbsp;
                <a href="https://github.com/binitp2" target="_blank"
...
def render_kpis(df: pd.DataFrame, session: dict, anomaly: dict):
    ...
    critical = len(df[df["priority"] == "CRITICAL"]
                   ) if not df.empty else session.get("critical_frames", 0)
    high = len(df[df["priority"] == "HIGH"]) if not df.empty else session.get(
        "high_alert_frames", 0)
    ...
    model_fitted = anomaly.get(
        "model_fitted", session.get("model_fitted", False))
...
def render_main(df: pd.DataFrame, session: dict, anomaly: dict,
    ...
    with col_ses:
        ...
        metrics = [
            ...
            ("Avg inference",
             f"{session.get('avg_inference_ms', 0):.1f} ms", ""),
            ("Avg anomaly",
             f"{session.get('avg_anomaly_ms', 0):.1f} ms",   ""),
            ("Avg preprocess",
             f"{session.get('avg_preprocess_ms', 0):.1f} ms", ""),
    ...
    if not df.empty:
        ...
        show_cols = [c for c in ["alert_id", "time_str", "frame_id", "priority",
                                 ...
                                 "reasons", "notified"]
                     if c in df.columns]
        display_df = df[df["priority"].isin(
            priority_filter)][show_cols].head(50)
        ...
        for row_idx, (_, row) in enumerate(display_df.iterrows()):
            ...
            rc[0].markdown(
                f'<div style="font-family:JetBrains Mono,monospace;font-size:0.72rem;color:{TEXT_SECONDARY};padding:0.25rem 0.3rem">{row.get("time_str", "—")}</div>', unsafe_allow_html=True)
            rc[1].markdown(
                f'<div style="font-family:JetBrains Mono,monospace;font-size:0.72rem;color:{CYAN};padding:0.25rem 0.3rem">F{int(frame_id):04d}</div>', unsafe_allow_html=True)
            rc[2].markdown(
                f'<div style="padding:0.25rem 0.3rem"><span class="alert-badge badge-{pri}">{pri}</span></div>', unsafe_allow_html=True)
            rc[3].markdown(
                f'<div style="font-family:JetBrains Mono,monospace;font-size:0.7rem;color:{color};padding:0.25rem 0.3rem">{row.get("anomaly_score", 0):.3f}</div>', unsafe_allow_html=True)
            rc[4].markdown(
                f'<div style="font-size:0.72rem;color:{TEXT_SECONDARY};padding:0.25rem 0.3rem">{row.get("detection_count", 0)}</div>', unsafe_allow_html=True)
            rc[5].markdown(
                f'<div style="font-size:0.72rem;color:{TEXT_SECONDARY};padding:0.25rem 0.3rem">{row.get("motion_score", "—") if row.get("motion_score") else "—"}</div>', unsafe_allow_html=True)
            rc[6].markdown(
                f'<div style="font-size:0.72rem;color:{TEXT_SECONDARY};padding:0.25rem 0.3rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="{reason}">{reason}</div>', unsafe_allow_html=True)

            with rc[7]:
                if notified:
                    st.markdown(
                        f'<div style="font-size:0.68rem;color:{GREEN};padding:0.3rem">✓ Notified</div>', unsafe_allow_html=True)
                else:
                    if st.button("📧 Notify", key=f"notify_{row_idx}_{aid}",
                        ...
                        if not op_email or not _is_valid_email(op_email):
                            st.toast(
                                "⚠️ Set your email in the sidebar first", icon="⚠️")
                        else:
                            with st.spinner("Sending..."):
                                ok, msg_out = _send_alert_email(
                                    row.to_dict(), op_email)
                            if ok:
                                load_alerts.clear()
                                written = _mark_notified(aid)
                                if written:
                                    st.toast(
                                        f"✅ Sent to {op_email} · Status: Notified", icon="✅")
                                else:
                                    st.toast(
                                        f"✅ Email sent · ⚠️ Could not update status (check terminal)", icon="⚠️")
                                st.rerun()
                            else:
        ...
        critical_unnotified = display_df[
            ...
            (display_df.get("notified", pd.Series(
                [False]*len(display_df))) == False)
        ]
                ...
                if st.button(f"🚨 Notify ALL Critical ({len(critical_unnotified)})",
                    ...
                    for _, r in critical_unnotified.iterrows():
                        ...
                        if ok:
                            ...
                            sent += 1
                    st.toast(
                        f"✅ {sent}/{len(critical_unnotified)} alerts sent and marked notified", icon="✅")
                    st.rerun()
